...
import ru.delimobil.cabbit.algebra.{Connection, DeliveryTag}
...
class Fs2RabbitSuite extends AnyFunSuite with BeforeAndAfterAll {
  ...
  test("publisher publishes") {
    TestUtils.channelWithPublishedMessage(connection)(List("hello from fs2-rabbit")) {
      case (declaration, _, queue) =>
  ...
  test("consumer gets") {
    ...
    TestUtils.channelWithPublishedMessage(connection)(List(message)) {
      case (declaration, consumer, queue) =>
        for {
  ...
  test("consumer consumes") {
    ...
    val Prefetched = 51
    val messages = (1 to MaxMessages).map(i => Random.nextString(Random.nextInt(100))).toList

    TestUtils.channelWithPublishedMessage(connection)(messages) {
      case (declaration, consumer, queue) =>
        for {
          ...
          streamedMessages <-
            stream
              .evalTap(delivery =>
                consumer.basicAck(DeliveryTag(delivery.getEnvelope.getDeliveryTag), multiple = false)
              )
              .take(MaxMessages)
              .compile
              .toList
